...
// figure out the function for the degrees/ pulse width line to use for the pulsewidth function
  ...
  m_servoPwmOut.pulsewidth_ms(((m_maxPulsewidthInMs-m_minPulsewidthInMs)
    /m_servoRangeInDegrees)*fixValue(degrees,
      0, m_servoRangeInDegrees)+m_minPulsewidthInMs);
 }

float TutorialServo::getServoRangeInDegrees() const { return m_servoRangeInDegrees;}

float TutorialServo::getMinPulseWidthInMs() const { return m_minPulsewidthInMs;}

float TutorialServo::getMaxPulseWidthInMs() const { return m_maxPulsewidthInMs;}