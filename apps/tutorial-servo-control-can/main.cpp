//Bonus: Enable the CAN peripheral filter to only read messages with your ID
...
bool readOnly(CANMSG message)
{
	if (message.id == SET_SERVO_RANGE)
		return true;
	else
		return false;
}
...
int main()
{
	...
	while (true)
	{
		if (can1.read(readOnly(msgrx)))
		{
			setPosition(msgrx);
		}
		else
			printf ("Wrong message ID or message ID not found!");
	}
}
*/
