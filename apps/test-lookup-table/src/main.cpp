...
int main() {
  ...
  const LookupTable::LookupTable<std::string, int> week = {
  ...
  printf("\r\n####### HOW TO INDEX #######\r\n");
  // No default case
  const LookupTable::LookupTable<std::string, int> a = {{"Jan", 1}, {"Feb", 2}};
  ...
  const LookupTable::LookupTable<uint8_t, uint8_t, static_cast<uint8_t>(42)> b = {{0, 100}, {1, 101}};
  printf("Existing Key: %d\r\n", b[0]);
  printf("Existing Key: %d\r\n", b[1]);
  printf("Non-Existing Key: %d\r\n", b[2]);
  printf("\r\n");
