...
BQ40Z80::BQ40Z80(PinName sda, PinName scl, uint8_t addr) : m_smbus(sda, scl, addr) {
  m_keys[0] = 0x0414;
  m_keys[1] = 0x3672;
  m_address = addr;
}
...
int BQ40Z80::manufacturer_read(const uint16_t sbs_cmd, void *data, const uint8_t length) {
  ...
  int status = m_smbus.block_write(SBS_MANUFACTURER_BLOCK_ACCESS, &command, sizeof(command), false);

  status = m_smbus.block_read(SBS_MANUFACTURER_BLOCK_ACCESS, data, length, false);

  // remove address bytes
  memmove(data, &((uint8_t *)data)[2], length - 2);
...
int BQ40Z80::manufacturer_write(const uint16_t sbs_cmd, void *data, const uint8_t length) {
  ...
  return m_smbus.block_write(SBS_MANUFACTURER_BLOCK_ACCESS, &buf, length + 2, false);
}
...
int BQ40Z80::getAllCellStatus() {
  ...
  m_cell_voltages[0] = ((uint16_t)((DAstatus1[1] << 8) | DAstatus1[0]) / 1000);
  m_cell_voltages[1] = ((uint16_t)((DAstatus1[3] << 8) | DAstatus1[2]) / 1000);
  m_cell_voltages[2] = ((uint16_t)((DAstatus1[5] << 8) | DAstatus1[4]) / 1000);
  m_cell_voltages[3] = ((uint16_t)((DAstatus1[7] << 8) | DAstatus1[6]) / 1000);

  m_cell_currents[0] = ((uint16_t)((DAstatus1[13] << 8) | DAstatus1[12]) / 1000);
  m_cell_currents[1] = ((uint16_t)((DAstatus1[15] << 8) | DAstatus1[14]) / 1000);
  m_cell_currents[2] = ((uint16_t)((DAstatus1[17] << 8) | DAstatus1[16]) / 1000);
  m_cell_currents[3] = ((uint16_t)((DAstatus1[19] << 8) | DAstatus1[18]) / 1000);

  m_cell_powers[0] = ((uint16_t)((DAstatus1[21] << 8) | DAstatus1[20]) / 1000);
  m_cell_powers[1] = ((uint16_t)((DAstatus1[23] << 8) | DAstatus1[22]) / 1000);
  m_cell_powers[2] = ((uint16_t)((DAstatus1[25] << 8) | DAstatus1[24]) / 1000);
  m_cell_powers[3] = ((uint16_t)((DAstatus1[27] << 8) | DAstatus1[26]) / 1000);

  m_total_power = ((uint16_t)((DAstatus1[29] << 8) | DAstatus1[28]) / 1000);
  m_avg_power   = ((uint16_t)((DAstatus1[31] << 8) | DAstatus1[30]) / 1000);
  ...
  m_cell_voltages[4] = ((uint16_t)((DAstatus3[1] << 8) | DAstatus1[0]) / 1000);
  m_cell_voltages[5] = ((uint16_t)((DAstatus3[7] << 8) | DAstatus1[6]) / 1000);
  m_cell_currents[4] = ((uint16_t)((DAstatus1[3] << 8) | DAstatus1[2]) / 1000);
  m_cell_currents[5] = ((uint16_t)((DAstatus1[9] << 8) | DAstatus1[8]) / 1000);
  m_cell_powers[4]   = ((uint16_t)((DAstatus1[5] << 8) | DAstatus1[4]) / 1000);
  m_cell_powers[5]   = ((uint16_t)((DAstatus1[11] << 8) | DAstatus1[10]) / 1000);
...
int BQ40Z80::getTemperatures() {
  ...
  m_temp_1                  = ((uint16_t)((DAstatus2[3] << 8) | DAstatus2[2]) / 1000);
...
int BQ40Z80::getStartupInfo() {
  ...
  status = manufacturer_read(SBS_MA_CMD::DEVICE_TYPE, &m_device_type, sizeof(m_device_type));
...
int BQ40Z80::flushLifetimeData() {
  return manufacturer_write(SBS_MA_CMD::LIFETIME_DATA_FLUSH, nullptr, 0);
}
...
int BQ40Z80::getDeviceType(uint16_t &data) {
...
int BQ40Z80::getHardwareVersion(uint16_t &data) {
  return manufacturer_read(SBS_MA_CMD::HARDWARE_VERSION, &data, sizeof(data));
}
...
int BQ40Z80::getCurrent(uint16_t &data) {
...
int BQ40Z80::getVoltage(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::VOLTAGE, data);
}

int BQ40Z80::getAvgCurrent(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::AVERAGE_CURRENT, data);
}

int BQ40Z80::getTimeToEmpty(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::RUN_TIME_TO_EMPTY, data);
}

int BQ40Z80::getAvgTimeToEmpty(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::AVERAGE_TIME_TO_EMPTY, data);
}

int BQ40Z80::getRemainingCapacity(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::REMAINING_CAPACITY, data);
}

int BQ40Z80::getMaxError(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::MAX_ERROR, data);
}

int BQ40Z80::getTemp(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::TEMPERATURE, data);
}

int BQ40Z80::getRelativeSOC(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::RELATIVE_STATE_OF_CHANGE, data);
}

int BQ40Z80::getAbsoluteSOC(uint16_t &data) {
  return m_smbus.read_word(SBS_CMD::ABS_STATE_OF_CHANGE, data);
}

int BQ40Z80::readDataFlash(const uint16_t address, void *data, const unsigned length) {
  int status = m_smbus.block_write(SBS_MANUFACTURER_BLOCK_ACCESS, &address, 2, false);

  status = m_smbus.block_read(SBS_MANUFACTURER_BLOCK_ACCESS, data, length, false);
  return status;
}

int BQ40Z80::writeDataFlash(const uint16_t address, void *data, const unsigned length) {
  uint8_t buf[MAC_DATA_BUF_SIZE + 2] = {};
  ...
  int status = m_smbus.block_write(SBS_MANUFACTURER_BLOCK_ACCESS, buf, length + 2, false);
