...
public:
  enum SERVO_TYPE {
    CONT_SERVO = 0,
    LIM_SERVO = 1
  }; // CONT_SERVO = 0 = CONTINUOUS Rotation; LIMITED_SERVO = 1 = LIMITED Range
     // Rotation
  ...
  bool setSpeed(float speed_);   // Sets rotation SPEED if Servo Type is
  // CONTINUOUS

  float read(void); // Returns SPEED if CONTINUOUS and POSITION if LIMITED
  float getMaxSpeed(void); // Returns maximum speed

  void setPeriod(int period); // Override default period (ONLY USE FOR SPECIFIC
  // FREQ REQUIREMENT)
...
protected:
  ...
  PwmOut
      pwm; // PWM object, does not have default constructor so have to use
           // INITIALIZER LIST to avoid COMPILER attempting to DEFAULT CONSTRUCT

  const int PWM_FREQ = 50, // DEFAULT PWM FREQUENCY, should work for both length
  ...
  enum SERVO_TYPE rotate_type; // Either CONTINUOUS or LIMITED rotation servo,
                               // see define statements.

  float range,   // RANGE OF MOTION, only valid for LIMIT_SERVO types
      max_speed, // MAXIMUM ROTATION SPEED in ANGLES PER SECOND, only valid for
